...
use oauth2::{AuthorizationCode, CsrfToken, Scope, reqwest::async_http_client};
use uuid::Uuid;

use crate::{config::CONFIG, db, login, token};

use super::{provider::Provider, query::User};
...
impl Mutation {
    async fn register(&self, username: String, password: String) -> Result<String> {
        let user = self.db.get::<db::User>().username(&username).execute().await?;
        match user {
            None => {
                self.db
                    .put(db::User {
    ...
    async fn create_token_auth_url(&self, ctx: &Context<'_>, provider: Provider, name: String, scopes: Vec<String>) -> Result<String> {
        let user = ctx.data_opt::<User>().ok_or_else(|| "Forbidden")?;
        ...
        let state = jwt::encode(&jwt::Header::default(), &claims, &jwt::EncodingKey::from_base64_secret(&CONFIG.state_key)?)?;
        let token::Claims { provider_id, scopes, .. } = claims;

        let (url, _) = CONFIG
            .providers
            .get(&provider_id)
            .ok_or_else(|| "invalid provider")?
    ...
    async fn create_token(&self, code: String, state: String) -> Result<NewToken> {
        let token_data = jwt::decode(&state, &jwt::DecodingKey::from_base64_secret(&CONFIG.state_key)?, &jwt::Validation::default())?;
        let token::Claims {
            name, provider_id, scopes, username, ..
        } = token_data.claims;

        let provider = CONFIG.providers.get(&provider_id).ok_or("an error occurred")?;
        let token = provider
            .oauth2_client(CONFIG.base_url.clone())
